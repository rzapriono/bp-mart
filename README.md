...
1. Buka command prompt dan masuk ke direktori proyek `bp-mart`, kemudian aktifkan virtual environment
...
## Melakukan routing pada proyek bp-mart agar dapat menjalankan aplikasi main
...
2. Isi `urls.py` dengan kode berikut
   ...
   ```
3. Buka file `urls.py` di dalam direktori proyek `bp-mart`
4. Impor fungsi `include` dari `django.urls.`
   ...
   ...
   ```
6. Tambahkan rute URL seperti berikut untuk mengarahkan ke tampilan `main` di dalam variabel `urlpatterns`
   ```python
   urlpatterns = [
      ...
...
3. Isi `main.html` untuk menampilkan data-data tersebut
   ...
   <h5>Name: </h5>
   <p>{{ creator_name }}</p> <!-- Ubahlah sesuai dengan nama kamu -->
   <h5>Class: </h5>
